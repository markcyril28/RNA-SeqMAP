...
if [[ -n "${MODULES_DIR:-}" && -f "${MODULES_DIR}/modules_loader.sh" ]]; then
    ...
    # Locate the project root by walking up until modules_gea/ is found, instead of a
    # fixed "../.." hop — keeps this working wherever the config tree lives
    # (config/ -> I_INPUTS/config/eggplant/... and any future relocation).
    _RD_ROOT="${PROJECT_ROOT:-}"
    if [[ -z "$_RD_ROOT" || ! -f "${_RD_ROOT}/modules_gea/modules_loader.sh" ]]; then
        _RD_ROOT="$_RUNTIME_DEFAULTS_DIR"
        while [[ -n "$_RD_ROOT" && "$_RD_ROOT" != "/" && ! -f "${_RD_ROOT}/modules_gea/modules_loader.sh" ]]; do
            _RD_ROOT="${_RD_ROOT%/*}"
        done
    fi
    if [[ ! -f "${_RD_ROOT}/modules_gea/modules_loader.sh" ]]; then
        echo "[ERROR] runtime_defaults.sh: could not locate modules_gea/modules_loader.sh above ${_RUNTIME_DEFAULTS_DIR}" >&2
        unset _RUNTIME_DEFAULTS_DIR _RD_ROOT
        return 1
    fi
    source "${_RD_ROOT}/modules_gea/modules_loader.sh"
    unset _RUNTIME_DEFAULTS_DIR _RD_ROOT
fi
