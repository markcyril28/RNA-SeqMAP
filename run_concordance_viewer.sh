...
#
# Usage:
#   bash run_concordance_viewer.sh                    # Generate viewer
#   bash run_concordance_viewer.sh --serve            # Generate + serve
#   bash run_concordance_viewer.sh --serve --port 9090
#   bash run_concordance_viewer.sh --manifest-only    # Only rebuild manifest
#   bash run_concordance_viewer.sh --open             # Generate + open
#   bash run_concordance_viewer.sh --output path.html # Custom output path
#   bash run_concordance_viewer.sh --dry-run          # Preview without changes
#
# Output:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --concordance-dir) shift; CONCORDANCE_DIR="$1" ;;
        --help|-h)
            sed -n '/^#/!q; s/^# \?//p' "$0"
            exit 0 ;;
        *)
...
if [[ "$DO_SERVE" == true ]]; then
    ...
    if [[ "$DRY_RUN" == true ]]; then
        log_info "[DRY RUN] Would execute: python3 -m http.server $SERVE_PORT --bind $SERVE_HOST --directory $CONCORDANCE_DIR"
    else
        python3 -m http.server "$SERVE_PORT" --bind "$SERVE_HOST" --directory "$CONCORDANCE_DIR"
    fi
fi
...
if [[ "$MANIFEST_ONLY" == true ]]; then
    ...
    log_info "  Local server : bash run_concordance_viewer.sh --serve"
fi
