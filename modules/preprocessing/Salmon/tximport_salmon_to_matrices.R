...
for (level_name in names(processing_levels)) {
  ...
  tx2gene_file <- file.path(INPUT_FASTAS_DIR, "mappings", paste0(MASTER_REFERENCE, ".fa.gene_trans_map"))
  # Try alternative naming conventions if not found
  if (!file.exists(tx2gene_file)) {
    tx2gene_file <- file.path(INPUT_FASTAS_DIR, "mappings", paste0(MASTER_REFERENCE, ".fasta.gene_trans_map"))
  }
