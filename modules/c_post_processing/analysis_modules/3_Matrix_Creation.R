...
import_stringtie <- function(ballgown_dir, sample_ids) {
  ...
  if (file.exists(gene_count_file)) {
    # fread is 10-50x faster than read.csv for large matrices
    df <- data.table::fread(gene_count_file, header = TRUE, data.table = FALSE)
    rownames(df) <- df[[1]]; df[[1]] <- NULL
    result$gene <- as.matrix(df)
  }
  if (file.exists(transcript_count_file)) {
    df <- data.table::fread(transcript_count_file, header = TRUE, data.table = FALSE)
    rownames(df) <- df[[1]]; df[[1]] <- NULL
    result$transcript <- as.matrix(df)
  }
  return(result)
}
