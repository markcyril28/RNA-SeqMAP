...
load_custom_gene_sets <- function(file_path) {
  ...
  gs_data <- if (.HAS_DATATABLE) {
    as.data.frame(data.table::fread(file_path, header = TRUE, sep = "\t"))
  } else {
...
create_wgcna_gene_sets <- function(wgcna_dir) {
  ...
  for (i in seq_along(module_files)) {
    mod_data <- if (.HAS_DATATABLE) {
      as.data.frame(data.table::fread(module_files[i], header = TRUE, sep = "\t"))
    } else {
      read.table(module_files[i], header = TRUE, sep = "\t", stringsAsFactors = FALSE)
    }
  ...
  # Filter NULL entries (modules that failed the colname check)
  module_list <- module_list[!vapply(module_list, is.null, logical(1))]
  if (length(module_list) == 0) return(NULL)
  all_modules <- if (.HAS_DATATABLE) {
    as.data.frame(data.table::rbindlist(module_list, use.names = TRUE))
  } else {
    do.call(rbind, module_list)
  }
...
run_gene_set_enrichment <- function(config = NULL, matrices_dir = NULL) {
  ...
  if (file.exists(full_file)) {
    ...
    full_df <- if (.HAS_DATATABLE) {
      as.data.frame(data.table::fread(full_file, header = TRUE, sep = "\t",
                                       select = 1L))
    } else {
      read.table(full_file, header = TRUE, sep = "\t", colClasses = col_classes,
