...
# Usage: source "modules_gea/modules_loader.sh"
#
# Structure:
...
MODULES_DIR="$(cd "$MODULES_DIR" && pwd)" || {
    echo "[ERROR] modules_loader.sh: Failed to resolve MODULES_DIR from BASH_SOURCE=${BASH_SOURCE[0]}" >&2
    return 1
}
export MODULES_DIR
...
source "$MODULES_DIR/logging/logging_utils.sh" || { echo "[ERROR] Failed to source logging_utils.sh from $MODULES_DIR/logging/" >&2; return 1; }
...
source "$MODULES_DIR/a_preprocessing/global_config_preproc.sh"   || { log_error "Failed to source global_config_preproc.sh"; return 1; }
source "$MODULES_DIR/a_preprocessing/shared_utils_preproc.sh"    || { log_error "Failed to source shared_utils_preproc.sh"; return 1; }
source "$MODULES_DIR/a_preprocessing/download.sh"                || { log_error "Failed to source download.sh"; return 1; }
source "$MODULES_DIR/a_preprocessing/trimming.sh"                || { log_error "Failed to source trimming.sh"; return 1; }
source "$MODULES_DIR/a_preprocessing/quality_checks.sh"          || { log_error "Failed to source quality_checks.sh"; return 1; }

# 3. Main methods modules
source "$MODULES_DIR/b_main_methods/global_config_method.sh"     || { log_error "Failed to source global_config_method.sh"; return 1; }
source "$MODULES_DIR/b_main_methods/shared_utils_method.sh"      || { log_error "Failed to source shared_utils_method.sh"; return 1; }
source "$MODULES_DIR/b_main_methods/methods_loader.sh"           || { log_error "Failed to source methods_loader.sh"; return 1; }
