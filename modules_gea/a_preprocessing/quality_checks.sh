	...
	if $_QC_HAS_FASTQC; then
		...
		if [[ -d "$RAW_DIR" ]]; then
			local _raw_qc=("$srr_outdir"/*_raw_fastqc.html); if [[ ! -f "${_raw_qc[0]:-}" ]]; then
				...
				run_with_space_time_log fastqc -t "${THREADS_PER_JOB:-${THREADS:-2}}" -o "$srr_outdir" \
		...
		if [[ -d "$TrimGalore_DIR" ]]; then
			local _trim_qc=("$srr_outdir"/*_trimmed_fastqc.html); if [[ ! -f "${_trim_qc[0]:-}" ]]; then
				...
				# Match both paired-end (*val*.fq*) and single-end (*_trimmed.fq*) naming
				local _trim_files=("$TrimGalore_DIR"/${SRR}*val*.fq* "$TrimGalore_DIR"/${SRR}*_trimmed.fq*)
				local _valid_trim_files=()
				for _tf in "${_trim_files[@]}"; do [[ -f "$_tf" ]] && _valid_trim_files+=("$_tf"); done
				if [[ ${#_valid_trim_files[@]} -gt 0 ]]; then
					run_with_space_time_log fastqc -t "${THREADS_PER_JOB:-${THREADS:-2}}" -o "$srr_outdir" \
						"${_valid_trim_files[@]}" 2>/dev/null || log_warn "FastQC failed for trimmed $SRR"
					rename_fastqc_outputs "$srr_outdir" "trimmed" "${_valid_trim_files[@]}"
				else
					log_warn "No trimmed files found for $SRR"
				fi
			fi
		fi
	else
...
run_quality_control_parallel() {
	...
	export _CONDA_PROFILE_SCRIPT ERROR_WARN_FILE LOG_FILE
	...
	parallel \
		...
		--env _CONDA_PROFILE_SCRIPT --env WF_MANAGED_ENV --env LOG_FILE --env ERROR_WARN_FILE \
		--env RAW_DIR_ROOT --env TRIM_DIR_ROOT --env FASTQC_ROOT --env THREADS_PER_JOB \
		-j "${JOBS:-2}" \
		--halt soon,fail,1 \
		--joblog "$FASTQC_ROOT/parallel_fastqc_${BASHPID:-$$}.log" \
		_qc_worker {} \
		< <(printf "%s\n" "${SRR_LIST[@]}")
}
